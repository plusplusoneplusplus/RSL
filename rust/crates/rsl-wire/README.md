...
A **pure-Rust implementation of the RSL Paxos wire format** — the marshaling
layer, the Rabin-64 message checksum, and every RSL message type across protocol
versions 1–6.

Production-Windows compatibility is tested against artifacts generated by
`RSLWindowsOracle`. The Linux proxy provides deterministic supplemental vectors
from selected production wire translation units; its packet, learn, and storage
models are not production authority.
...
The crate is validated by independent harnesses, all run by `cargo test`:

1. **`windows_oracle`** — consumes the production Windows artifact and verifies
   schema, checksums, parsing, versions and lengths.
2. **`linux_proxy_vectors`** — for every Linux proxy corpus record: unmarshal succeeds,
   ...
   **`containers`** additionally rebuilds the 7 raw `MarshalData`
   ...
   byte-identically.
3. **`fields`** — each message is **independently reconstructed** from the
...
4. **`proptest`** — arbitrary valid messages across all versions reach a byte
   fixpoint under marshal/parse.
5. **`fuzz_smoke`** — pseudo-random and corpus-mutated bytes never panic
